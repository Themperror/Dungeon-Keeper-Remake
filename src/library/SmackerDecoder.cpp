...
namespace Smacker {
	...
	*/
	...
	class SmallHuffmanTree {
	public:
		SmallHuffmanTree(BitStreamMemory8LSB &bs);

		uint16 getCode(BitStreamMemory8LSB &bs);
	private:
		enum {
			SMK_NODE = 0x8000
		};
		...
		BitStreamMemory8LSB &_bs;
	};

	SmallHuffmanTree::SmallHuffmanTree(BitStreamMemory8LSB &bs)
		: _treeSize(0), _bs(bs) {
	...
	uint16 SmallHuffmanTree::getCode(BitStreamMemory8LSB &bs) {
		byte peek = bs.peekBits(MIN(bs.size() - bs.pos(), 8));
	...
	*/
	...
	class BigHuffmanTree {
	public:
		BigHuffmanTree(BitStreamMemory8LSB &bs, int allocSize);
		~BigHuffmanTree();

		void reset();
		uint32 getCode(BitStreamMemory8LSB &bs);
	private:
		enum {
			SMK_NODE = 0x80000000
		};
	...
	BigHuffmanTree::BigHuffmanTree(BitStreamMemory8LSB &bs, int allocSize)
		: _bs(bs) {
		uint32 bit = _bs.getBit();
		if (!bit) {
			_tree = new uint32[1];
	...
	uint32 BigHuffmanTree::getCode(BitStreamMemory8LSB &bs) {
		byte peek = bs.peekBits(MIN(bs.size() - bs.pos(), 8));
	...
	bool SmackerDecoder::loadStream(SeekableReadStream *stream) {
		close();
		...
		SmackerVideoTrack *videoTrack = new SmackerVideoTrack(width, height, frameCount, frameRate, _header.flags, _header.signature);
		video = videoTrack;
		...
		for (i = 0; i < 7; ++i) 
		{
			...
			if (_header.audioInfo->hasAudio)
			{
				SmackerAudioTrack *audioTrack = new SmackerAudioTrack(_header.audioInfo[i]);
				audio[i] = audioTrack;
			}
		}
		...
		BitStreamMemory8LSB bs(new BitStreamMemoryStream(huffmanTrees, _header.treesSize, true), true);
	...
	{
		SmackerVideoTrack *videoTrack = (SmackerVideoTrack *)getVideoTrack();
		...
		BitStreamMemory8LSB bs(new BitStreamMemoryStream(frameData, frameDataSize + 1, true), true);
	...
	void SmackerDecoder::handleAudioTrack(byte track, uint32 chunkSize, uint32 unpackedSize) 
	{
		...
		if (_header.audioInfo[track].hasAudio) {
			...
			SmackerAudioTrack *audioTrack = (SmackerAudioTrack *)getAudioTrack(track);
			...
			if (_header.audioInfo[track].compression == kCompressionRDFT || _header.audioInfo[track].compression == kCompressionDCT) {
				...
				audioTrack->DecompressDPCM(soundBuffer, chunkSize + 1, unpackedSize);
				free(soundBuffer);
			}
			else 
			{
				// Uncompressed audio (PCM)
				audioTrack->_buffer = soundBuffer;
				audioTrack->_bufferSize = chunkSize;
			}
		}
		else {
	...
	SmackerVideoTrack::SmackerVideoTrack(uint32 width, uint32 height, uint32 frameCount, float frameRate, uint32 flags, uint32 signature)
	{
		_surface = new Surface();
		_surface->create(width, height * (flags ? 2 : 1), PixelFormat::createFormatCLUT8());
	...
	SmackerVideoTrack::~SmackerVideoTrack() {
		_surface->free();
		delete _surface;
	...
	uint16 SmackerVideoTrack::getWidth() const {
		return _surface->w;
	}

	uint16 SmackerVideoTrack::getHeight() const {
		return _surface->h;
	}

	PixelFormat SmackerVideoTrack::getPixelFormat() const {
		return _surface->format;
	}

	void SmackerVideoTrack::readTrees(BitStreamMemory8LSB &bs, uint32 mMapSize, uint32 mClrSize, uint32 fullSize, uint32 typeSize) {
	...
	void SmackerVideoTrack::decodeFrame(BitStreamMemory8LSB &bs) {
		_MMapTree->reset();
		_MClrTree->reset();
		_FullTree->reset();
		_TypeTree->reset();
	...
	void SmackerVideoTrack::unpackPalette(SeekableReadStream *stream) {
	...
	SmackerAudioTrack::SmackerAudioTrack(const AudioInfo &audioInfo)
	{
		_audioInfo = audioInfo;
	}

	SmackerAudioTrack::~SmackerAudioTrack()
	{

	}


	void SmackerAudioTrack::DecompressDPCM(byte *buffer, uint32 bufferSize, uint32 unpackedSize) {
		BitStreamMemory8LSB audioBS(new BitStreamMemoryStream(buffer, bufferSize), true);
		...
		for (int i = 0; i < (isStereo ? 2 : 1); i++, curPointer += (is16Bits ? 2 : 1), curPos += (is16Bits ? 2 : 1)) {
			if (is16Bits)
				WRITE_LE_UINT16(curPointer, bases[i]);
			else
		...
		while (curPos < unpackedSize) {
			...
			if (!is16Bits) {
				...
				for (int k = 0; k < (isStereo ? 2 : 1); k++) {
					...
					WRITE_LE_UINT16(curPointer, bases[k]);
					curPointer += 2;
					curPos += 2;
				}
			}
		...
		_buffer = unpackedBuffer;
		_bufferSize = unpackedSize;
	}
