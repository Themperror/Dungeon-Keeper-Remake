...
namespace Themp
{
	...
	private:
		...
		void LoadCreatures();
		void LoadLowResLevelGUITextures();
		void LoadHiResLevelGUITextures();
		void LoadGUITextures(std::wstring datFile, std::wstring tabFile, std::vector<GUITab>& tabVector, std::vector<GUITexture>& guiTexVector);
		void LoadStrings(std::wstring datFile, std::wstring language);
		void LoadLowResMenuGUITextures();
		void LoadHiResMenuGUITextures();
	};
};
