...
bool LevelConfig::LoadConfiguration()
{
	...
	for (std::string line; std::getline(data, line);)
	{
		...
		{ //creature data
			...
			//assign temporaries
			c.CreatureNumber = (uint8_t)CreatureNumber;
			c.HealReq = (uint8_t)HealReq;
			c.HealThreshold = (uint8_t)HealThreshold;
			c.Strength = (uint8_t)Strength;
			c.Armor = (uint8_t)Armor;
			c.Dexterity = (uint8_t)Dexterity;
			c.Fear = (uint8_t)Fear;
			c.Defence = (uint8_t)Defence;
			c.Luck = (uint8_t)Luck;
			c.Recovery = (uint8_t)Recovery;
			c.HungerFill = (uint8_t)HungerFill;
			c.LairSize = (uint8_t)LairSize;
			c.HurtByLava = (uint8_t)HurtByLava;
			c.BaseSpeed = (uint8_t)BaseSpeed;
			c.SleepExperienceSlab = (uint8_t)SleepExperienceSlab;
			c.SleepExperience = (uint8_t)SleepExperience;
			c.ScavengeValue = (uint8_t)ScavengeValue;
			c.ExperienceForHitting = (uint8_t)ExperienceForHitting;
			c.TrainingValue = (uint8_t)TrainingValue;
			c.TrainingCost = (uint8_t)TrainingCost;
			c.VisualRange = (uint8_t)VisualRange;
			c.ScavengerCost = (uint8_t)ScavengerCost;
			c.GrowUp = (uint8_t)GrowUp;
			c.GrowUpLevel = (uint8_t)GrowUpLevel;
			c.MaxAngleChange = (uint8_t)MaxAngleChange;
			c.WalkingAnimSpeed = (uint8_t)WalkingAnimSpeed;
			c.AttackPreference = (uint8_t)AttackPreference;
			c.Hearing = (uint8_t)Hearing;
			c.EntranceRoom1 = (uint8_t)EntranceRoom1;
			c.EntranceRoom2 = (uint8_t)EntranceRoom2;
			c.EntranceRoom3 = (uint8_t)EntranceRoom3;
			c.Room1SlabsRequired = (uint8_t)Room1SlabsRequired;
			c.Room2SlabsRequired = (uint8_t)Room2SlabsRequired;
			c.Room3SlabsRequired = (uint8_t)Room3SlabsRequired;
			c.JobStress = (uint8_t)JobStress;
			c.ResearchValue = (uint8_t)ResearchValue;
			c.HeroVsKeeper = (uint8_t)HeroVsKeeper;
			c.ManufactureValue = (uint8_t)ManufactureValue;
			c.SlapsToKill = (uint8_t)SlapsToKill;
			c.CreatureLoyalty = (uint8_t)CreatureLoyalty;
			c.LoyaltyLevel = (uint8_t)LoyaltyLevel;
			c.LairEnemy = (uint8_t)LairEnemy;
			c.EyeEffect = (uint8_t)EyeEffect;
			c.Rebirth = (uint8_t)Rebirth;
			c.DamageToBoulder = (uint8_t)DamageToBoulder;
			c.Power[0] = (uint8_t)Power[0];
			c.Power[1] = (uint8_t)Power[1];
			c.Power[2] = (uint8_t)Power[2];
			c.Power[3] = (uint8_t)Power[3];
			c.Power[4] = (uint8_t)Power[4];
			c.Power[5] = (uint8_t)Power[5];
			c.Power[6] = (uint8_t)Power[6];
			c.Power[7] = (uint8_t)Power[7];
			c.Power[8] = (uint8_t)Power[8];
			c.Power[9] = (uint8_t)Power[9];
			c.PowerLevelRequired[0] = (uint8_t)PowerLevelRequired[0];
			c.PowerLevelRequired[1] = (uint8_t)PowerLevelRequired[1];
			c.PowerLevelRequired[2] = (uint8_t)PowerLevelRequired[2];
			c.PowerLevelRequired[3] = (uint8_t)PowerLevelRequired[3];
			c.PowerLevelRequired[4] = (uint8_t)PowerLevelRequired[4];
			c.PowerLevelRequired[5] = (uint8_t)PowerLevelRequired[5];
			c.PowerLevelRequired[6] = (uint8_t)PowerLevelRequired[6];
			c.PowerLevelRequired[7] = (uint8_t)PowerLevelRequired[7];
			c.PowerLevelRequired[8] = (uint8_t)PowerLevelRequired[8];
			c.PowerLevelRequired[9] = (uint8_t)PowerLevelRequired[9];
